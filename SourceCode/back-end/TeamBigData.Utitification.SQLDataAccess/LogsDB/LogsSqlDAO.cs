...
{
    public class LogsSqlDAO : DbContext, ILogsDBInserter, ILogsDBUpdater
    {
        ...
        public async Task<Response> UpdateLogsTest(String sqlCommand)
        {
            var connection = new SqlConnection(_connectionString);
            var command = new SqlCommand(sqlCommand, connection);
            command.Parameters.Add(new SqlParameter("@m", "Updated"));
            ...
            if (response.errorMessage.Equals("Nothing Affected"))
            {
                ...
                response.errorMessage = "No Log updated.";
            }
            else if (response.isSuccessful)
            {
                response.isSuccessful = true;
                response.errorMessage = "Log has been updated";
            }
