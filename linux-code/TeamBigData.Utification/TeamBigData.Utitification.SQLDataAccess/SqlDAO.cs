    ...
    public class SqlDAO : IDBInserter, IDBCounter, IDAO, IDBSelecter
    {
        ...
        public Task<Response> DeleteUser(UserProfile user)
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                //     result.errorMessage = e.Message;
                // }
                // var deleteSql = "DELETE FROM dbo.Users WHERE username = '" + user._username + "';";
                var deleteSql = "DELETE FROM dbo.Users WHERE username = '" + user._username + "';";
                try
                {
                    var command = new MySqlCommand(deleteSql, connection);
        ...
        public Task<bool> IsValidUsername(String username)
        {
            ...
            {
                connect.Open();
                var command = new MySqlCommand(sqlStatement, connect);
        ...
        public Task<Response> CreateRecoveryRequest(String username, String newPassword)
        {
            ...
            {
                connect.Open();
                var command = new MySqlCommand(insertSql, connect);
                try
                {
        ...
        public Task<Response> GetNewPassword(string username)
        {
            ...
            using (MySqlConnection connect = new MySqlConnection(_connectionString))
            {
                ...
                try
                {
                    var command = new MySqlCommand(sqlSelect, connect);
        ...
        public Task<Response> RequestFulfilled(string username)
        {
            ...
            using (MySqlConnection connect = new MySqlConnection(_connectionString))
            {
                ...
                try
                {
                    var command = new MySqlCommand(fulfill, connect);
                    if (command.ExecuteNonQuery() > 1)
                    {
                        result.isSuccessful = true;
