...
{
    //same issue lacks white space and comments
    public class PinService
    {
        //private field of connection strings of databases
        public async Task<List<Pin>> GetPinTable(string userHash)
        {
            var tcs = new TaskCompletionSource<List<Pin>>();
            List<Pin> pins = new List<Pin>();
            //Integrated Security=True;Encrypt=False is not production ready and exposes the system to vulnerabilities
            //connection strings should be comming from a configuration file
            //dll can be reverse engineered
            ...
            //check for userHash since it can break a function if its used as a parameter
            //have validations to exit early to run as efficiently as possible
            if (result.Count== 0)
            {
