...
namespace TeamBigData.Utification.SQLDataAccess
{
    public class SqlDAO : IDBInserter, IDBCounter, IDAO, IDBSelecter, IDBUpdater, IDBAnalysis
    {
        ...
        public Task<List<Pin>> SelectPinTable()
        {
            ...
            using (SqlConnection connect = new SqlConnection(_connectionString))
            {
                try
                {
                    //open async
                    connect.Open();
                    //create a sql reader directly
                    using (var reader = (new SqlCommand(sqlStatement, connect)).ExecuteReader())
                    {
                        // read through all rows is bad using a while loop
                        // use a data reader since its more efficient
                        // sql data reader that takes in sql command and is optimized for sql
                        while (reader.Read())
                        {
                            //redundant to set into not used values then reset 
                            int pinID = 0;
                            int userID = 0;
                            string lat = "";
                            string lng = "";
                            ...
                            //create a pin with paramaeters above
                            Pin temp = new Pin(pinID, userID, lat, lng, pinType, description, disabled, completed, dateTime);
                            //then add onto pins list
                            pins.Add(temp);
                            //creates memory over flow if has large amount of rows
                            //retieve data in partition so its easier for the system
                            //get the data in chunks on demand
                            //no point of returning 100% of the data
                        }
                        
                        reader.Close();
                    }
                    //automatically closes since its in a using statement for both
                    connect.Close();
                }
                catch (SqlException s)
                {
        ...
        public Task<Response> InsertPin(Pin pin)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                // try should start here since connection can fail
                connection.Open();
                //Creates an Insert SQL statements using the collumn names and values given
                //do not hard code insert and use sql commands add parameter to dynamically generate a sql statement
                //better to use a stored procedure and has baked in sanitizations is not required to return a value
                //functionalities in the databases
                //
