...
namespace TeamBigData.Utification.PinServices
{
    ...
    {
        private readonly IDBInserter _insertSqlDAO;
        private readonly IDBSelecter _selectSqlDAO;
        private readonly Logger _logger;
        public PinService(SqlDAO sqlDAO, Logger logger) 
        {
            _insertSqlDAO = sqlDAO;
            _selectSqlDAO = sqlDAO;
            _logger = logger;
        }
        ...
        public async Task<List<Pin>> GetPinTable(string userHash)
        {
            ...
            var result = await _selectSqlDAO.SelectPinTable().ConfigureAwait(false);
            if (result.Count== 0)
            {
        ...
        public async Task<Response> StoreNewPin(CreatePinDto pin, string userHash)
        {
            ...
            //var logger = new Logger(new SqlDAO(@"Server=.\;Database=TeamBigData.Utification.Logs;User=AppUser;Password=t;TrustServerCertificate=True;Encrypt=True"));
            var result = await _insertSqlDAO.InsertPin(pin).ConfigureAwait(false);
